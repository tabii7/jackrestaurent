...
public function storePaymentDetails(Request $request)
{
    ...
    $orderType = session('order.type');
    $orderDate = session('order.details.date');
    $orderTime = session('order.details.time');
    $orderPostcode = session('order.details.postcode');
    ...
    $payment = Payment::create([
        ...
        'order_type' => $orderType, // Save order type (Pickup or Delivery)
        'order_date' => $orderDate, // Save order date (for Pickup or Delivery)
        'order_time' => $orderTime, // Save order time (for Pickup)
        'order_postcode' => $orderPostcode, // Save postcode (for Delivery)
    ]);

    // Clear session data if necessary
    session()->forget('guest_addresses');
    session()->forget('cart');
    session()->forget('order'); // Forget the order session as well
...
public function savePickupDetails(Request $request)
{
    ...
    $finalPickupDate = $futureDate ?? $pickupDate;

    // Save pickup details and title in session
    session([
        'order' => [
            'type' => 'Pickup',
            'details' => [
                'date' => $finalPickupDate,
                'time' => $pickupTime,
            ],
        ],
    ]);

    return redirect()->route('shop.index')->with('success', 'Pickup details saved successfully!');
}
...
public function saveDeliveryDetails(Request $request)
{
    ...
    // Save delivery details and title in session
    session([
        'order' => [
            'type' => 'Delivery',
            'details' => [
    ...
    return redirect()->route('shop.index')->with('success', 'Delivery details saved successfully!');
}
