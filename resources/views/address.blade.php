...
@section('content')
                ...
                <div class="row g-lg-4 g-4">
                                            ...
                                            <div class="address-details">
                                                ...
                                                <div class="option-section">
                                                    ...
                                                    <a href="javascript:void(0)"
                                                        class="btn gray-btn rounded-2 mt-0 deliver-btn"
                                                        data-address-id="{{ $address->id ?? $loop->index }}"
                                                        @if (
                                                            (isset($address->selected) && $address->selected == 1) ||
                                                                (isset($address['selected']) && $address['selected'] == 1)) class="btn-black"
                                                style="background-color: black; color: white;" @endif>
                                                        @if (
                                                            (isset($address->selected) && $address->selected == 1) ||
                                                                (isset($address['selected']) && $address['selected'] == 1))
                                                            Selected
                    ...
                    <div class="col-lg-4">
                        <div class="order-summery-section sticky-top">
                            ...
                            <a href="{{ route('payment') }}"
                                class="btn theme-btn restaurant-btn w-100 rounded-2" id="proceed-to-payment">PROCEED TO PAYMENT</a>

                            <script>
                                // Calculate and update the total price when a coupon is selected
                                document.querySelectorAll('input[name="coupon_id"]').forEach(function (radio) {
                                    radio.addEventListener('change', function() {
                                        ...
                                        // If a valid coupon is selected, apply the discount
                                ...
                                <img class="dots-design" src="{{ asset('assets/images/svg/dots-design.svg') }}"
                                    alt="dots">
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="address-details" tabindex="-1" aria-labelledby="address-details-label"
        aria-hidden="true">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#address-' + addressId + ' .deliver-btn').addClass('btn-black').removeClass(
                            'gray-btn');
